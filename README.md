...
### Вариант 60: Объявление и инициализация строковой константы на языке Rust.
...
1. **Уникальность идентификаторов:**
   ...
   * *Пример ошибки:* `const NAME: &str = "GFG"; const NAME: &str = "GFG";`
   * *Сообщение:* `Ошибка: идентификатор 'NAME' уже объявлен. Строка:1, Поз:26`

### Структура AST
Основные типы узлов:

- `ConstDeclStr` — корень объявления.
- `name` — идентификатор.
- `modifiers` — ключевое слово const.
- `type` — Тип данных.
- `str` — строка.
...
4. **Запуск:**
Готовый исполняемый файл, не требующий установки .NET Runtime, находится по пути:
`./bin/Release/net8.0-windows/win-x64/publish/WinFormsApp4.exe`
...
- `using System.Windows.Forms;` — Форма для рисования графа AST.
- `System.Drawing (GDI+)` — Основной ресурс, который использовался для построения.
- `AdjustableArrowCap` — Класс для создания наконечников на ребрах, позволяющий создать структуру «родитель -> потомок».

### Как открыть граф AST

1. Запустить анализатор на корректном входе без синтаксических ошибок.
2. Нажать пункт `Показать AST`.
3. Откроется отдельная форма с деревом.
